...
/* ---------- ascension heat: the dark burns hotter above your first win ---------- */
const heatMonMult = () => 1 + 0.08 * (G.heat || 0);
const heatScoreMult = () => 1 + 0.1 * (G.heat || 0);
const heatDread = () => 10 * (G.heat || 0);
function heatUnlocked(classId) {
  try { return (JSON.parse(localStorage.getItem('arcaneHeat') || '{}') || {})[classId] || 0; } catch (e) { return 0; }
}
function heatSel(classId) {
  try {
    const sel = (JSON.parse(localStorage.getItem('arcaneHeatSel') || '{}') || {})[classId] || 0;
    return Math.min(sel, heatUnlocked(classId));
  } catch (e) { return 0; }
}
function cycleHeat(classId) {
  const max = heatUnlocked(classId);
  if (max <= 0) return;
  try {
    const sels = JSON.parse(localStorage.getItem('arcaneHeatSel') || '{}') || {};
    sels[classId] = ((sels[classId] || 0) + 1) % (max + 1);
    localStorage.setItem('arcaneHeatSel', JSON.stringify(sels));
  } catch (e) { /* ignore */ }
  updateHeatChips();
}
function updateHeatChips() {
  for (const card of document.querySelectorAll('.class-card')) {
    const cid = card.dataset.class;
    let chip = card.querySelector('.cc-heat');
    const max = heatUnlocked(cid);
    if (max <= 0) { if (chip) chip.remove(); continue; }
    if (!chip) {
      chip = document.createElement('div');
      chip.className = 'cc-heat';
      chip.title = 'won with this hero: the dark can burn hotter — click to set the heat (foes +8%/heat, the hunt comes sooner, score +10%/heat)';
      chip.addEventListener('click', ev => { ev.stopPropagation(); cycleHeat(cid); });
      card.appendChild(chip);
    }
    const sel = heatSel(cid);
    chip.textContent = sel > 0 ? `🔥${sel}` : '🔥·';
    chip.style.opacity = sel > 0 ? '1' : '0.55';
  }
}
function recordHeatUnlock() {
  try {
    const h = JSON.parse(localStorage.getItem('arcaneHeat') || '{}') || {};
    const next = Math.min(5, (G.heat || 0) + 1);
    if ((h[G.classId] || 0) < next) {
      h[G.classId] = next;
      localStorage.setItem('arcaneHeat', JSON.stringify(h));
      addMsg(`THE DARK BURNS HOTTER — heat ${next} unlocked for the ${G.classDef.name}.`, 'm-gold');
    }
  } catch (e) { /* ignore */ }
}

const score = () => Math.round(G.diff.scoreMult * heatScoreMult() * (
...
function newGame(classId) {
  ...
  G.heat = G.daily ? 0 : (G.nextHeat || 0); // dailies stay the fair seed — heat never touches them
  G.nextHeat = 0;
  if (G.heat > 0) addMsg(`HEAT ${G.heat} — the dark is ${G.heat * 8}% crueler and hunts sooner. Score pays +${G.heat * 10}%.`, 'm-gold');
...
function spawnMonster(id, x, y) {
  ...
  const hp = Math.max(1, Math.round(d.hp * G.diff.monHp * heatMonMult() * deep));
  const atk = Math.max(1, Math.round(d.atk * G.diff.monAtk * heatMonMult() * deep));
  G.monsters.push({
...
function afterPlayerTurn() {
  ...
  const dreadAt = G.diff.dreadAt - heatDread() - dreadShift();
...
function updateUI() {
  const p = G.player;
  if (!p) return;
  $('hero-name').textContent = 'THE ' + G.classDef.name.toUpperCase() + (G.heat > 0 ? ' 🔥' + G.heat : '');
  ...
  if (G.depth !== FINAL_DEPTH && G.floorTurns > G.diff.dreadAt - heatDread() - dreadShift()) statusBits.push('<span style="color:var(--red)">⌛ the dark hunts you</span>');
  else if (G.depth !== FINAL_DEPTH && G.floorTurns > G.diff.dreadAt - heatDread() - dreadShift() - 30) statusBits.push('<span style="color:#c98a5e">⌛ the dark stirs…</span>');
...
function saveRun() {
  ...
  try {
    ...
    localStorage.setItem(SAVE_KEY, JSON.stringify({
      ...
      corpses: G.corpses, prostrated: !!G.prostrated, relicFloor: G.relicFloor || 0, relicGiven: G.relicGiven || 0, reads: G.reads || {}, heat: G.heat || 0,
      map: {
...
function recordRun(won) {
  ...
  try {
    ...
    const key = `${G.classId}_${G.diffId}`;
    r = led[key] || r;
    if (won) { r.wins += 1; if ((G.heat || 0) > (r.heat || 0)) r.heat = G.heat; }
    const sc = score();
...
function updateRecordLine() {
  ...
  for (const [key, r] of Object.entries(led)) {
    ...
    byClass[cls] = byClass[cls] || { wins: 0, heat: 0, details: [] };
    byClass[cls].wins += r.wins;
    if ((r.heat || 0) > byClass[cls].heat) byClass[cls].heat = r.heat;
  ...
  for (const [cls, agg] of Object.entries(byClass)) {
    ...
    bits.push(`<span title="${agg.details.join(' · ')}">⚔ ${cname} ×${agg.wins}${agg.heat > 0 ? ' 🔥' + agg.heat : ''}</span>`);
  }
  el.classList.toggle('hidden', bits.length === 0);
...
for (const card of document.querySelectorAll('.class-card')) {
  card.addEventListener('click', () => {
    Sfx.ensure(); Music.start();
    G.nextHeat = heatSel(card.dataset.class);
